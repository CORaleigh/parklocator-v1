...
function GetFacilityIds(title){
    ...
    CreateParkClasses(title);
	/*actSelect = null;
	classSelect = null;
	dateSelect = null;
	reclink = null;
	var requestHandle = esri.request({
		url:'http://mapstest.raleighnc.gov/ParkLocator/facilities.txt',
		load:function(response, io){
			$.each(response, function(index, value){
				if (value.park == title){
					esri.request({
						url:'https://maps.raleighnc.gov/arcgis/rest/services/Parks/ParkLocator/MapServer/exts/ParkLocatorSOE/ProgramSearch',
                        data:{name:value.park},//'http://mapstest.raleighnc.gov/class/class.php?ids='+value.ids,
...
function CreateParkClasses(park){
	RemoveChildren(dojo.byId('divParkClasses'));
...
function BuildClassSelection(tr,park)
{
    $.get("https://maps.raleighnc.gov/arcgis/rest/services/Parks/ParkLocator/MapServer/exts/ParkLocatorSOE/ProgramSearch",
        {name:park,f:'json'},function (data){
            var programs = $.parseJSON(data);
            $(tr).append("<div id='sectionDiv'><select id='sectionSel' class='classSelects'></select></div>");
            $("#sectionSel").append("<option value='prompt'>Select a section...</option>");
            $.each(programs.Programs.Sections, function(i,section){
                $("#sectionSel").append("<option>"+section.Name+"</option>");
            })
            ...
            $("#sectionSel").change(function(){
                ...
                $("#timeDiv").remove();
                $("#classLink").remove();
                $("#sectionSel option[value='prompt']").remove();
                var index = $(this).prop("selectedIndex");
                section = programs.Programs.Sections[index];
                $(tr).append("<div id='activityDiv'><select id='activitySel' class='classSelects'></select></div>");
                $("#activitySel").append("<option value='prompt'>Select an activity...</option>");  

                $.each(section.Activities, function(i,activity){
                    $("#activitySel").append("<option>"+activity.Name+"</option>");
                });
                ...
                $("#activitySel").change(function(){
                    ...
                    $("#timeDiv").remove();
                    $("#classLink").remove();
                    $("#activitySel option[value='prompt']").remove();
                    var index = $(this).prop("selectedIndex");
                    activity = section.Activities[index];
                    $(tr).append("<div id='courseDiv'><select id='courseSel' class='classSelects'></select></div>");
                    $("#courseSel").append("<option value='prompt'>Select a course...</option>");   
                    $.each(activity.Courses, function(i,course){
                        $("#courseSel").append("<option>"+course.Name+"</option>");
                    });

                    $("#courseSel").change(function(){
                        $("#timeDiv").remove();
                        $("#classLink").remove();
                        $("#courseSel option[value='prompt']").remove();
                        var index = $(this).prop("selectedIndex");
                        course = activity.Courses[index];
                        $(tr).append("<div id='timeDiv'><select id='timeSel' class='classSelects'></select></div>");
                        $("#timeSel").append("<option value='prompt'>Select a start time...</option>"); 
                        $.each(course.Times, function(i,time){
                            $("#timeSel").append("<option data-aid='"+activity.ID+"' data-cid='"+time.ID+"'>"+time.Start+"</option>");
                        });     

                        $("#timeSel").change(function(){
                            $("#timeSel option[value='prompt']").remove();
                            var aid = $("#timeSel option:selected").data("aid");
                            var cid = $("#timeSel option:selected").data("cid");
                            $(tr).append("<a id='classLink' href='http://reclink.raleighnc.gov/Activities/ActivitiesCourseDetails.asp?aid="+aid+"&cid="+cid+"' target='_blank'>Sign Up</a>");

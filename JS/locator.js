    ...
    if (!(dojo.byId('txtAddress').disabled)) {
        if (dojo.byId('spanAddress').className == 'text') {
            if (dojo.byId("txtAddress").value.trim() == "") {
                ...
                ShowDialog('Locator Error', 'Enter address to Locate');
                return;
            }
            GeoCodeAddress(evt);
        }
            ...
            if (dojo.byId("txtAddress").value.trim() == '' || dojo.byId("txtAddress").value == dojo.byId('txtAddress').title) {
                dojo.byId('txtAddress').value = '';
                dojo.byId('txtAddress').focus();
                ShowDialog('Locator Error', 'Enter park or activity name to Locate');
                return;
            }
            if (dojo.byId("txtAddress").value.trim() != "") {
                var findParams;
                ...
                ShowDialog('Error', 'Enter an Park / Activity to search');
            }
        }
    }
}

function SearchByName(parkName){
	routeText = " ";
	if (parkName.trim() != "") {
		var findParams;
		findParams = new esri.tasks.FindParameters();
		...
		findParams.searchFields = searchFields;
		findParams.outSpatialReference = map.spatialReference;
		findParams.searchText = parkName;
...
function PopulateSearchItem(featureSet) {
    ...
    dojo.byId('divParkActivityContainer').style.borderBottom = '#FFFFFF 1px solid';
    ...
    if (features.length > 0) {
        ...
        for (var i = 0; i < features.length; i++) {
            ...
            if (i == 0) {
                td1.style.backgroundColor = "#083e01"//"#3C4824";
            }
            td1.onclick = function (evt) {
                ...
                this.style.backgroundColor = "#083e01"//"#3C4824";
                newLeftOffice = 0;
...
function FindTaskResults(results) {
    ...
    if (results.feature.attributes[facilityName].toLowerCase() != 'null') {
        featureID = results.feature.attributes[facilityName];//objectId];
    }
    else {
    ...
    if (!mapPoint) {
        map.centerAndZoom(results.feature.geometry, results.feature.attributes['Zoom Scale']);//map._slider.maximum - 2);
    }
    HideRipple();    
...
function CreateActivityList() {
    ...
    dojo.byId('divActivityListContainer').style.borderBottom = '#FFFFFF 1px solid';
    ...
    if (fieldSearch.length != 0) {
        ...
        aTask.execute(query, function (featureSet) {
            ...
            if (featureSet.features.length > 0) {
                ...
                for (var i = 0; i < featureSet.features.length; i++) {
                    ...
                    td1.onclick = function (evt) {
                        ...
                        featureID = featureSet.features[ss].attributes[infoWindowHeader[0].FieldName];//map.getLayer(devPlanLayerID).objectIdField];
                        if (!mapPoint) {
                            map.centerAndZoom(highlightPoint, featureSet.features[ss].attributes['Zoom Scale']);//map._slider.maximum - 2);
                        }
                        HideRipple();
            ...
            if (parkCount == 0) {
                newLeftOffice = 0;
                featureID = featureSet.features[0].attributes[infoWindowHeader[0].FieldName];//map.getLayer(devPlanLayerID).objectIdField];
                if (!mapPoint) {
                    map.centerAndZoom(featureSet.features[0].geometry, featureSet.features[0].attributes['Zoom Scale']);//map._slider.maximum - 2);
                }
                HideRipple();
...
function GeoCodeAddress(evt) {
    ...
    if (!validAddressFlag) {
        ...
        ShowDialog('Locator Error', 'Please enter address in [Address, City, State, Zip] or in [Address, Zip] format.');
        return;
    }
    if (evt.id != 'directionSearch') {
...
function ShowLocatedAddress(candidates, evt) {
    ...
    if (candidates.length > 0) {
        ...
        ShowDialog('Locator Error', 'Unable to locate specified address.');
        return;
    }
}
...
function buildWhereClause(){
	...
	for (var i = 0;i < activities.length;i++)
	{
		...
		if (activity.checked)
		{
			clauses.push(activity.value + " = 'Yes'");
		}
	}
	var clause = "";
	//var joiner = (dojo.byId('amenitiesSelect').value == 'With All')?' AND ':' OR ';
	var select = dojo.byId('amenitiesSelect');
	var joiner = (select.options[select.selectedIndex].value == 'With All')?' AND ':' OR ';
	for (var j=0;j < clauses.length;j++)
	{
		if (j != clauses.length - 1)
		{
			clause += clauses[j]+joiner;
		}
		else
		{
			clause += clauses[j];
...
function CreateDistance(featureset) {
    ...
    for (var i = 0; i < featureset.features.length; i++) {
        ...
        if (i == (featureset.features.length - 1)) {
            ...
            ConfigureRoute(mapPoint);
            map.getLayer(highlightPollLayerId).clear();
            featureID = features[0].attributes[infoWindowHeader[0].FieldName];//map.getLayer(devPlanLayerID).objectIdField];
            HideRipple();
...
function CreateParkList(features) {
    ...
    dojo.byId('divParkListContainer').style.borderBottom = '#FFFFFF 1px solid';
    ...
    for (var i = 0; i < features.length; i++) {
        ...
        if (i == 0) {
            tdparkList.style.backgroundColor = "#083e01"//"#3C4824";
        }
        tdparkList.style.height = "20px";
        tdparkList.innerHTML = "<b>"+(i+1).toString()+".</b> "+features[i].attributes[infoWindowHeader[0].FieldName];// + '&nbsp(' + features[i].dist + ' miles)';
        ...
        tdparkList.onclick = function (evt) {
            ...
            featureID = features[point].attributes[infoWindowHeader[0].FieldName];//map.getLayer(devPlanLayerID).objectIdField];
            HideRipple();
            GlowRipple(highlightPoint);
            newLeftOffice = 0;
            CreateParkDetails(this, features);
...
function CreateParkDetails(point, features) {
	dojo.byId('divParkDetails').style.display = 'block';	
    dojo.byId('divParkDirections').style.display = 'block';
    RemoveChildren(dojo.byId('divParkDetails'));
    var s;
    if (point) {
    ...
    tdHeader.style.backgroundColor = "#083e01"//"#3C4824";
    ...
    if (!features[s].feature) {
		...
		title = features[s].attributes[infoWindowHeader[0].FieldName];
		url = features[s].attributes['URL'];
		phone = features[s].attributes['PHONE'];
		parkScale = features[s].attributes['SCALE'];
    }
    else {
        span.innerHTML = features[s].feature.attributes[infoWindowHeader[0].Alias];
		title = features[s].feature.attributes[infoWindowHeader[0].Alias];
		url = features[s].feature.attributes['Website'];
		phone = features[s].feature.attributes['Telephone Number'];
		parkScale = features[s].feature.attributes['Zoom Scale'];
    }
	
	GetFacilityIds(title);
    ...
    container.style.backgroundColor = "#6eb43e";//"#849966";
    container.style.width = "95%";
	//JG changed height from 112 to 300
    container.style.height = 152 + "px";

    var divInfoWindow = document.createElement("div");
    divInfoWindow.id = "divInfoWindows";
    divInfoWindow.style.overflow = "hidden";
	divInfoWindow.style.marginLeft = "10px";
	//JG changed height from 112 to 300
    divInfoWindow.style.height = 250 + "px";
	...
	photoImg.innerHTML = "<img src='images/photos/"+title.trim().replace(/"/g,'') +".jpg' style='width:150px;float:left;padding-top:5px;'/>";
	...
	if (phone != "-"){
		phoneDiv.innerHTML = "<br/><span style='padding-left:5px'>"+phone+"</span>";
		photoImg.appendChild(phoneDiv);	
	}
	if (url != "-")
	{
		link.innerHTML = "<br/><a href='"+url+"' target='_blank' style='padding-left:5px'>Park Website</a>";
		photoImg.appendChild(link);	
	}
	

	
	var zoomlink = document.createElement('div');
	zoomlink.innerHTML = "<br/><a href='#' style='padding-left:5px'>Zoom To Park</a>";
	dojo.connect(zoomlink, "onclick", function(evt){
		map.centerAndZoom(pollPoint, parkScale);//map._slider.maximum - 2);
	});
	photoImg.appendChild(zoomlink);
    ...
    for (var w = 4; w > 0; w--) {
        ...
        bottomline.style.backgroundColor = "#6eb43e";//"#849966";
...
function CreateParkDirections(point, features) {
    ...
    if (!features[s].feature) {
		...
		pollName = features[s].attributes[infoWindowHeader[0].FieldName];
    }
    else {
        pollPoint = features[s].feature.geometry;
		pollName = features[s].feature.attributes[infoWindowHeader[0].Alias];
    }
    ...
    if (!layer) {
        ...
        text.style.backgroundColor = "#083e01"//"#3C4824";
...
var classes;
var actSelect, classSelect, dateSelect, reclink;

function GetFacilityIds(title){
	RemoveChildren(dojo.byId('divParkClasses'));
	actSelect = null;
	classSelect = null;
	dateSelect = null;
	reclink = null;
	var requestHandle = esri.request({
		url:'https://maps.raleighnc.gov/ParkLocator/facilities.txt',
		load:function(response, io){
			$.each(response, function(index, value){
				if (value.park == title){
					esri.request({
						url:'https://maps.raleighnc.gov/class/class.php?ids='+value.ids,
						load: function(response2, io){
							classes = response2;
							if(classes){
								CreateParkClasses();
							}
...
function CreateParkClasses(){
	RemoveChildren(dojo.byId('divParkClasses'));
    var mainContainer = dojo.byId('divParkClasses');
    mainContainer.style.marginBottom = '8px';
    mainContainer.style.marginLeft = '10px';
    var bold = document.createElement('b');
    bold.className = 'cbottom';
    ...
    var tableHeader;
    var tBodyHeader;
    if (!dojo.byId('tblAddressDetailsClass')) {
        tableHeader = document.createElement('table');
        tBodyHeader = document.createElement('tbody');
        tableHeader.id = 'tblAddressDetailsClass';
        tableHeader.style.width = "95%";
        tBodyHeader.id = 'tBodyAddressDetailsClass';
        tableHeader.cellSpacing = 0;
        tableHeader.cellPadding = 0;
        tableHeader.appendChild(tBodyHeader);
    }
    else {
        tableHeader = dojo.byId('tblAddressDetailslass');
        tBodyHeader = dojo.byId('tBodyAddressDetailslass');
    }
    var trHeader = document.createElement('tr');
    trHeader.id = "trparkHeaderDetailsClass";
    tBodyHeader.appendChild(trHeader);
    var tdHeader = document.createElement('td');
    tdHeader.id = "tdparkHeaderDetailsClass";
    tdHeader.style.height = "30px";
    tdHeader.style.backgroundColor = "#083e01"//"#3C4824";
    tdHeader.style.paddingLeft = '10px';
    trHeader.appendChild(tdHeader);
    var span = document.createElement('span');
    span.id = 'leftInfoWindowHeaderClass';
    span.style.fontWeight = 'bolder';
	span.innerHTML = "Programs";
	tdHeader.appendChild(span);
	
    var container = document.createElement('div');
    container.id = "scrollbar_container3";
    container.style.overflow = "hidden";
    container.style.position = "relative";
    container.style.backgroundColor = "#6eb43e";//"#849966";
    container.style.width = "95%";
	...
	sectionSelect.className = "classSelects";
	$(sectionSelect).append($("<option></option>").attr('value','prompt').text('Select Section'));
	//sectionSelect.appendChild("<option value='prompt'>Select Section</option>");
	$.each(classes, function(index,value){
		var section = $.trim(value.SECTION);
		if ($.inArray(section, sectionArr) == -1){
			sectionArr.push(section);
			$(sectionSelect).append($("<option></option>").text(section));
		};								
	});
	$(sectionSelect).change(function(){
		/*if(dojo.byId('actSelect')){
			tr.removeChild(actSelect);
		}
		if(dojo.byId('classSelect')){
			tr.removeChild(classSelect);
		}*/
		
		if ($('#sectionSelect option:selected').val() != 'prompt'){
			$("#sectionSelect option[value='prompt']").remove();		
		}
		var sectionSel = $('#sectionSelect option:selected').text();
		
		var actArr = [];
		if (!actSelect){
			actSelect = document.createElement("select");
			actSelect.id = "actSelect";
			actSelect.className = "classSelects";
			tr.appendChild(actSelect);	
		...
		if (reclink){
			$(reclink).css('display','none');
		}
		
		$(actSelect).empty();
		$(actSelect).append($("<option></option>").attr('value','prompt').text('Select Activity'));
		
		$.each(classes, function(index, value){
			var activity = $.trim(value.ACTIVITY);
			
			if($.inArray(activity,actArr) == -1){
				if (sectionSel == $.trim(value.SECTION)){
					actArr.push(activity);
					$(actSelect).append($("<option></option>").text(activity));				
				}
		...
		$(actSelect).change(function(){
			...
			if ($('#actSelect option:selected').val() != 'prompt'){
				$("#actSelect option[value='prompt']").remove();		
			}
			var actSel = $('#actSelect option:selected').text();
			var classArr = [];
			
			if (!classSelect){
				classSelect = document.createElement("select");
				classSelect.id = "classSelect";	
				classSelect.className = "classSelects";
				tr.appendChild(classSelect);					
			}
			...
			$(classSelect).empty();
			$(classSelect).append($("<option></option>").attr('value', 'prompt').text('Select Class'));
			$.each(classes, function(index, value){
				var className = $.trim(value.COURSE);
				if($.inArray(className,classArr) == -1){
					if(actSel == $.trim(value.ACTIVITY)){
						classArr.push(className);
						$(classSelect).append($("<option></option>").text(className));					
					}
			...
			$(classSelect).change(function(){
				$("#classSelect option[value='prompt']").remove();
				
				var classSel = $('#classSelect option:selected').text();
				var dateArr = [];
				
				if(!dateSelect){
					dateSelect = document.createElement("select");
					dateSelect.id = "dateSelect";	
					dateSelect.className = "classSelects";
					tr.appendChild(dateSelect);	
				}
				...
				$(dateSelect).empty();
				$(dateSelect).append($("<option></option>").attr('value', 'prompt').text('Select Start Time'));		
				
				$.each(classes, function(index, value){
					var date = $.trim(value.START_DATE);
					if($.inArray(date,dateArr) == -1){
						if(classSel == $.trim(value.COURSE)){
							dateArr.push(date);
							$(dateSelect).append($("<option></option>").attr('data-value','{"cid":'+value.COURSE_ID+',"aid":'+value.ACTIVITY_ID+'}').text(date));					
						}
				...
				$(dateSelect).change(function(){
					$("#dateSelect option[value='prompt']").remove();
					var aid = $('#dateSelect').find(":selected").data('value').aid;
					var cid = $('#dateSelect').find(":selected").data('value').cid;
					if(!reclink){
						reclink = document.createElement("a");
						reclink.id = "reclink";
						tr.appendChild(reclink);
					}
					$(reclink).css('display','block');
					$(reclink).attr('href', "https://reclink.raleighnc.gov/reclink/Activities/ActivitiesCourseDetails.asp?aid="+aid+"&cid="+cid).attr('target','_blank').text('View Class Details');

					//window.open("https://reclinkdev/class71/Activities/ActivitiesCourseDetails.asp?aid="+aid+"&cid="+cid);
				});
	...
	tr.appendChild(sectionSelect);
	

	

    var divInfoWindow = document.createElement("div");
    divInfoWindow.id = "divInfoWindowsClass";
    divInfoWindow.style.overflow = "hidden";
	//JG changed height from 112 to 300
    divInfoWindow.style.height = 250 + "px";
    divInfoWindow.className = 'scrollbar_content';

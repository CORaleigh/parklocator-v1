...
function ToggleApplication() {
	...
	var layerNode = dojo.byId('divLayersTitleContainer');
    if (dojo.coords(layerNode).h > 0) {
        WipeOutControl(layerNode, 500);
        dijit.byId('imgLayers').attr("checked", false);
    }
...
function ShareLink(site) {
    ...
    if (featureID) {        
            var urlStr = encodeURI(url.path) + "?park=" + featureID;//"?featureID=" + pollPoint.x + "," + pollPoint.y + "," + featureID;
        }
        else {
            var urlStr = encodeURI(url.path) + "?extent=" + mapExtent;
        }
    url = urlStr.replace(/\s/g,'+');//dojo.string.substitute(tinyURLServiceURL.URL, [urlStr]);
	switch (site) {
		case "facebook":
			window.open(dojo.string.substitute(tinyURLServiceURL.FacebookShareURL, [url]));
			break;
		case "twitter":
			window.open(dojo.string.substitute(tinyURLServiceURL.TwitterShareURL, [url]));
			break;
		case "mail":
			parent.location = dojo.string.substitute(tinyURLServiceURL.MailShare, [url]);
			break;
	}
    /*dojo.io.script.get({
        url: url,
...
function ExecuteQueryTask() {
    ...
    queryTask.execute(query, function (fset) {       
        if (fset.features.length > 0) {
            ...
            map.centerAndZoom(fset.features[0].geometry, fset.features[0].attributes['SCALE']);//map._slider.maximum - 2);
...
function getMapExtent() {
    var extents = Math.round(map.extent.xmin).toString() + ",";
    extents += Math.round(map.extent.ymin).toString() + ",";
    extents += Math.round(map.extent.xmax).toString() + ",";
    extents += Math.round(map.extent.ymax).toString();
    return (extents);
}
...
function CloseDialog() {
    ...
    if (dijit.byId('imgGeolocation')) {
        if (dijit.byId('imgGeolocation').checked) {
            ...
            zdojo.byId('rbAddress').disabled = false;
...
//function to slide right panel
function AnimateDetailsViewRight() {
    var node = dojo.byId('divRightPanelBackground');
    if (dojo.window.getBox().w - dojo.coords(node).l == 320) {
		//var divBackground = dojo.animateProperty({node:node, duration: 1000, properties:{width:{end:0}}});
		//var divContent = dojo.animateProperty({node:dojo.byId('divSearchPanel'), duration: 1000, properties:{width:{end:0}}});
		//var imageToggle = dojo.fx.slideTo({ node: dojo.byId('divToggleDetailRight'), duration: 1000, properties: { right: { end: 0, unit: "px"}} });	
		var divBackground = dojo.animateProperty({node:node, duration:1000, properties: {width:0}});
        var divBackground = dojo.fx.slideTo({ node: node, duration: 1000, properties: { right: { end: -320, unit: "px"}} });
        var divContent = dojo.fx.slideTo({ node: dojo.byId('divSearchPanel'), duration: 1000, properties: { right: { end: -320, unit: "px"}} });
        var imageToggle = dojo.fx.slideTo({ node: dojo.byId('divToggleDetailRight'), duration: 1000, properties: { right: { end: 0, unit: "px"}} });
        //var mapSlider = dojo.fx.slideTo({ node: dojo.byId('map_zoom_slider'), duration: 1000, properties: { right: { end: 15, unit: "px"}} });
        var combinedAnimation = dojo.fx.combine([divBackground, divContent, imageToggle]);
        dojo.connect(combinedAnimation, "onEnd", function () {
            dojo.byId('divToggleDetailRight').className = "divToggleDetailExpandRight";
        });
        combinedAnimation.play();
		//divBackground.play();
    }
    else {
		//var divBackground = dojo.animateProperty({node:node, duration: 1000, properties:{width:{end:320}}});
		//var divContent = dojo.animateProperty({node:dojo.byId('divSearchPanel'), duration: 1000, properties:{width:{end:320}}});
		//var imageToggle = dojo.fx.slideTo({ node: dojo.byId('divToggleDetailRight'), duration: 1000, properties: { right: { end: 320, unit: "px"}} });	
		var divBackground = dojo.animateProperty({node:node, duration:1000, properties: {width:320}});
        var divBackground = dojo.fx.slideTo({ node: node, duration: 1000, properties: { right: { end: 0, unit: "px"}} });
        var divContent = dojo.fx.slideTo({ node: dojo.byId('divSearchPanel'), duration: 1000, properties: { right: { end: 0, unit: "px"}} });
        var imageToggle = dojo.fx.slideTo({ node: dojo.byId('divToggleDetailRight'), duration: 1000, properties: { right: { end: 320, unit: "px"}} });
        //var mapSlider = dojo.fx.slideTo({ node: dojo.byId('map_zoom_slider'), duration: 1000, properties: { right: { end: 335, unit: "px"}} });
        var combinedAnimation = dojo.fx.combine([divBackground, divContent, imageToggle]);
		
        dojo.connect(combinedAnimation, "onEnd", function () {
            dojo.byId('divToggleDetailRight').className = "divToggleDetailCollapseRight";
        });
        combinedAnimation.play();
		//divBackground.play();
    }
...
function SetSearchControlFields(spandisabled, spanenabled, title, value, rbControl) {
   ...
   // dojo.byId("txtAddress").defaultText = value;
...
function ShowMyLocation(evt) {
    if (dijit.byId('imgGeolocation').checked) {
		...
		function (error) {
		    ...
		    switch (error.code) {
		        case error.TIMEOUT:
		            ShowDialog('Error', 'Timeout');
		            break;
		        case error.POSITION_UNAVAILABLE:
		            ShowDialog('Error', 'Position unavailable');
		            break;
            ...
            ShowDialog('Error', 'Browser do not support GeoLocation');
            return;
        }
    }
    else {
...
function ShowCurrentLocation(evt) {
    if (dijit.byId('btnCurrentLocation').checked) {
		dijit.byId('btnCurrentLocation').attr("checked", false);
...
function ShowInfoWindow(feature, point) {
    ...
    for (var i in devPlanLayer.fields) {
        ...
        if (devPlanLayer.fields[i].type == "esriFieldTypeDate") {
            if (attributes[devPlanLayer.fields[i].name]) {
                attributes[devPlanLayer.fields[i].name] = dojo.date.locale.format(new Date(attributes[devPlanLayer.fields[i].name]), { datePattern: "MMM dd, yyyy", selector: "date" });
            }
        }
    }
	...
	//added by Justin Greco 5/7/2012
	var photoImg = document.createElement('div');
	photoImg.innerHTML = "<br/><img src='images/photos/"+title+".jpg' style='width:200px;height:150px'/><br/>";
	divInfoWindow.appendChild(photoImg);
	...
	if (url != "-")
	{
		link.innerHTML = "<br/><a href='"+feature.attributes['URL']+"' target='_blank'>Website</a></br>";
		divInfoWindow.appendChild(link);	
	}
...
function GetDistance(startPoint, endPoint) {
	//NEED TO CONVERT FROM STATEPLANE
    ...
    dist = dist * 60 * 1.1515;
    distance = (dist * 10) / 10;*/
	distance = Math.sqrt(Math.pow(Math.abs(startPoint.x - endPoint.x),2)+Math.pow(Math.abs(startPoint.y - endPoint.y),2))/5280;
}
...
//function to hide LayersWidget onmouseout
function HideLayersWidget() {
    dijit.byId('imgLayers').attr("checked", false);
...
function AddOpLayers(){
	for (var i=0;i < opLayerCollection.length;i++)
	{
		...
		if (opLayerCollection[i].Type == "dynamic"){

			var layer = new esri.layers.ArcGISDynamicMapServiceLayer(opLayerCollection[i].MapURL, {id:opLayerCollection[i].Key, visible:isVisible});
			if (opLayerCollection[i].VisibleLayers){
				layer.setVisibleLayers(opLayerCollection[i].VisibleLayers.split(","));
			}
			map.addLayer(layer);
		}
		else if (opLayerCollection[i].Type == "tiled"){

			var layer = new esri.layers.ArcGISTiledMapServiceLayer(opLayerCollection[i].MapURL, {id:opLayerCollection[i].Key, visible:isVisible});
			map.addLayer(layer);
		}
		else if (opLayerCollection[i].Type == "feature"){
			var layer = new esri.layers.FeatureLayer(opLayerCollection[i].MapURL, {id:opLayerCollection[i].Key, visible:isVisible, mode: esri.layers.FeatureLayer.MODE_ONDEMAND});
			map.addLayer(layer);
		}
	}
}
...
function GetParkList(){
	var list = [];
	var query = new esri.tasks.Query();
	query.outFields = ['NAME','ALIAS1','ALIAS2'];
	...
	queryTask.execute(query, function(results){
		for (var i = 0;i < results.features.length;i++){
			...
			if (feature.attributes['NAME']){
				list.push({name:feature.attributes['NAME'], value:feature.attributes['NAME']});
				if (feature.attributes['ALIAS1']){
					list.push({name:feature.attributes['ALIAS1'], value:feature.attributes['NAME']});
				}
				if (feature.attributes['ALIAS2']){
					list.push({name:feature.attributes['ALIAS2'], value:feature.attributes['NAME']});
				}				
			}
		}
		...
		//var parkStore = new dojo.store.Memory({data: list});
        list.unshift({name:"Select Park...", value:"PROMPT"});
		var parkStore = new dojo.data.ItemFileReadStore({data:{"items":list}});
		var filteringSelect = new dijit.form.FilteringSelect({
            ...
            autoComplete:true,
			onChange:function(){
                if(dojo.byId('nameInput').value != "PROMPT"){
                    SearchByName(dojo.byId('nameInput').value);                   
                }
			}},
			"nameInput");
	});
...
function PrintOld(){
	var gp = new esri.tasks.Geoprocessor("http://gisarcweb1/ArcGIS/rest/services/Geoprocessing/ParkPrint/GPServer/Park%20Locator%20Print%20Tool");
	var title = "";
	if (pollPoint){
		title = "Directions to "+dojo.byId('leftInfoWindowHeader').innerHTML;
	}else{
		title = "Park Locator Map";}
	scale = esri.geometry.getScale(map);
	extent = [];
	extent.push(map.extent.xmin);
	extent.push(map.extent.ymin);
	extent.push(map.extent.xmax);
	extent.push(map.extent.ymax);
	basemap = "";
	directions = "-";
	var showPoint = false;
	var showLine = false;
	...
	for (var i=0;i < opLayerCollection.length;i++)
	{
		layer = map.getLayer(opLayerCollection[i].Key);
		if (layer.visible){
			layers.push(opLayerCollection[i].Name);
		}
	}
	
	for (var j=0;j < baseMapLayerCollection.length;j++){
		layer = map.getLayer(baseMapLayerCollection[j].Key);
		if (layer.visible){
			basemap = baseMapLayerCollection[j].Name;
			break;
		}
	}
	
	var point = new esri.tasks.FeatureSet();
	point.features = [];
	var line = new esri.tasks.FeatureSet();
	line.features = [];

	if (mapPoint){
		point.features.push(new esri.Graphic(mapPoint));
		showPoint = true;
	}
	if (pollRoute){
		line.features.push(new esri.Graphic(pollRoute));
		extent = [];
		extent.push(pollRoute.getExtent().xmin);
		extent.push(pollRoute.getExtent().ymin);
		extent.push(pollRoute.getExtent().xmax);
		extent.push(pollRoute.getExtent().ymax);				
		directions = routeText;
		showLine = true;
	}	
	...
	var params = {
		...
		"Point":(point.features.length > 0)?point:"",
		"Route":(line.features.length > 0)?line:"",
		"ShowPoint":showPoint,
		"ShowRoute":showLine,
		"env:outSR":2264,
		"env:processSR":2264
	};
	ShowLoadingMessage('Printing...');
	gp.submitJob(params, function(jobInfo){
		if (jobInfo.jobStatus != "esriJobFailed"){
			gp.getResultData(jobInfo.jobId, "URL", function(file){
...
function Print(){
	//var url = 'http://gisdevarc1:6080/arcgis/rest/services/Printing/ParkLocatorPrint/GPServer/Export%20Web%20Map';
	dijit.byId('imgPrint').attr("checked", false);
	var url = 'https://maps.raleighnc.gov/arcgis/rest/services/Parks/ParkLocatorPrint/GPServer/Export%20Park%20Map';
	var title = "Park Locator Map";
	if (pollPoint){
		title = dojo.byId('leftInfoWindowHeader').innerHTML;
	}
	if (pollRoute){
		title = "Directions to "+dojo.byId('leftInfoWindowHeader').innerHTML;
	}
	...
	map.getLayer('highlightPollLayerId').visible = false;
	params.map = map;
	ShowLoadingMessage('Printing...');
	var legLayer1 = new esri.tasks.LegendLayer();
	legLayer1.layerId = "amenities_buildings";
	var legLayer2 = new esri.tasks.LegendLayer();
	legLayer2.layerId = "amenities_outdoor";
	var template = new esri.tasks.PrintTemplate();
	template.format = "PDF";
	template.layout = "Letter ANSI A Landscape";
	template.layoutOptions = {titleText:title,scalebarUnit:"Miles",legendLayers:[legLayer1, legLayer2]};

	params.extraParameters = {Directions: routeText.replace(/;/g,'\r\n')};
	...
	map.getLayer('highlightPollLayerId').visible = true;

}
function ClearSearch(){
	dijit.byId('imgClear').attr("checked", false);
	...
	dojo.byId('divParkDetails').style.display = 'none';	
	dojo.byId('divParkDirections').style.display = 'none';
	dojo.byId('divParkClasses').style.display = 'none';
	dojo.byId('txtAddress').value = "222 W Hargett St";
    dojo.byId('nameInput').value = "";
	featureID = null;
	mapPoint = null;
	pollPoint = null;
	pollRoute = null;
	...
	map.getLayer(tempGraphicsLayerId).clear();
	map.graphics.clear();
    if (dojo.byId('divToggleDetail').className == "divToggleDetailExpand"){
        AnimateDetailsView();        
    }

    var activities = activityContainer.getChildren();
